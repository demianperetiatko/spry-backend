...
def kpi_avg_daily_meetings_time(
    events: list, prev_events: list, count_work_day: int, count_people: int = 1
) -> dict:
    avg_daily_meetings_time = calculate_avg_daily_meetings_hour(
        events, count_work_day * count_people
    )
    prev_avg_daily_meetings_time = calculate_avg_daily_meetings_hour(
        prev_events, count_work_day * count_people
    )
...
def kpi_avg_daily_meetings_cost(
    events: list, prev_events: list, members: list, count_work_day, currency
) -> dict:
    if currency:
        total_cost = calculate_total_events_cost(events, members) / (
            len(members) * count_work_day
        )
        prev_total_cost = calculate_total_events_cost(prev_events, members) / (
            len(members) * count_work_day
        )
...
def kpi_avg_member_meetings_cost(
    events: list, prev_events: list, members: list, currency
) -> dict:
    if currency:
...
def kpi_person_total_time_percent(
    events: list, prev_events: list, count_work_day: int
) -> dict:
    ...
    return _calculate_percentage_kpi(
        current_time, previous_time, total_work_hours, is_positive_change=False
    )


def kpi_person_deep_work_time_percent(
    events: list, prev_events: list, count_work_day: int
) -> dict:
    ...
    return _calculate_percentage_kpi(
        current_time, previous_time, total_work_hours, is_positive_change=True
    )


def kpi_person_buffers_time_percent(
    events: list, prev_events: list, count_work_day: int
) -> dict:
...
def kpi_person_transition_time_percent(
    events: list, prev_events: list, count_work_day: int
) -> dict:
...
def kpi_team_total_time_percent(
    events: list, prev_events: list, count_work_day: int, team_members_count: int
) -> dict:
    current_time = calculate_total_events_duration(events)
    previous_time = calculate_total_events_duration(prev_events)
...
def kpi_team_deep_work_time_percent(
    events: list, prev_events: list, count_work_day: int, team_members_count: int
) -> dict:
    current_time = calculate_team_deep_work_time(
        events, count_work_day, team_members_count
    )
    previous_time = calculate_team_deep_work_time(
        prev_events, count_work_day, team_members_count
    )
    total_work_hours = count_work_day * WORKDAY_HOURS * team_members_count
...
def kpi_team_buffers_time_percent(
    events: list, prev_events: list, count_work_day: int, team_members_count: int
) -> dict:
    current_time = calculate_buffer_time(events)
    previous_time = calculate_buffer_time(prev_events)
...
def kpi_team_transition_time_percent(
    events: list, prev_events: list, count_work_day: int, team_members_count: int
) -> dict:
    current_time = calculate_transition_time(events)
    previous_time = calculate_transition_time(prev_events)
